...
const makeDom = async()=>{
    let dom;
    const pluginCheck = await fetch('/api/plugins/pluginmanager/', { method:'HEAD' });
    if (!pluginCheck.ok) {
        dom = document.createElement('div'); {
            const heading = document.createElement('h3'); {
                heading.textContent = 'Server Plugin Missing';
                dom.append(heading);
            }
            const text = document.createElement('div'); {
                text.textContent = 'Manually install the Plugin Manager Plugin and restart the SillyTavern server.';
                dom.append(text);
            }
            const link = document.createElement('a'); {
                link.href = 'https://github.com/LenAnderson/SillyTavern-PluginManagerPlugin';
                link.textContent = 'https://github.com/LenAnderson/SillyTavern-PluginManagerPlugin';
                link.target = '_blank';
                dom.append(link);
            }
        }
